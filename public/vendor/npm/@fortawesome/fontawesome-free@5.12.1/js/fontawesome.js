  ...
  (function() {
    ...
    function _arrayWithoutHoles(arr) {
      if (Array.isArray(arr)) {
        for (var i = 0,
            arr2 = new Array(arr.length); i < arr.length; i++)
          arr2[i] = arr[i];
        return arr2;
      }
    }
    ...
    var parse = {transform: function transform(transformString) {
    ...
    var layer = function layer(assembler) {
      ...
      return apiObject({type: 'layer'}, function() {
        ...
        return [{
          tag: 'span',
          attributes: {class: ["".concat(config.familyPrefix, "-layers")].concat(_toConsumableArray(classes)).join(' ')},
          children: children
        }];
      });
    };
